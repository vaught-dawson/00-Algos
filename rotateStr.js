...
function rotateStr(str, amount) {
  ...
  var startOfRotation = str.length - amount;

  for (let i = startOfRotation; i < str.length; i++) {
    newOutputString += str[i];
  }
  for (let i = 0; i < startOfRotation; i++) {
    newOutputString += str[i];
  }
...
function isRotation(s1, s2) {
  ...
  let tempStr = s1 + s1;
  if (tempStr.includes(s2)) return true;
  return false;
}

function recursiveTryRotation(s1, s2, rec) {
  if (rec >= s1.length) {
    return false;
  }
  let temp = rotateStr(s2, rec);
  if (temp == s1) return true;
  return recursiveTryRotation(s1, s2, ++rec);
}
